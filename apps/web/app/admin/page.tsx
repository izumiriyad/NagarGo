...
export default function Admin() {
  ...
  // Allow DashboardTab quick-action buttons to switch tabs via custom event
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent<string>).detail as Tab;
      if (detail) setTab(detail);
    };
    window.addEventListener("admin:tab-switch", handler);
    return () => window.removeEventListener("admin:tab-switch", handler);
  }, []);
