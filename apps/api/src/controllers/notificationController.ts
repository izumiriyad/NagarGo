...
export async function deleteNotification(req: Request, res: Response) {
  const recipientType = recipientTypeForRole(req.auth!.role);
  const result = await Notification.findOneAndDelete({ _id: req.params.id, recipientType, recipientId: req.auth!.sub });
...
export async function clearAllNotifications(req: Request, res: Response) {
  const recipientType = recipientTypeForRole(req.auth!.role);
  const { deletedCount } = await Notification.deleteMany({ recipientType, recipientId: req.auth!.sub });
