...
export async function createOrder(req: Request, res: Response) {
  ...
  const pricing = await calculatePricing({ cityId: body.cityId, distanceKm: route.distanceKm, estimatedMinutes: route.estimatedMinutes, isEmergency: body.isEmergency });
...
/** Customer lists their own orders, newest-first, paginated with optional status/date filters. */
export async function listMyOrders(req: Request, res: Response) {
  ...
  const [orders, total] = await Promise.all([
    Order.find(filter)
      ...
      .skip(skip)
      .limit(limit)
      .select("publicId status pickup destination pricing isEmergency createdAt ratedByCustomer paymentId"),
...
export async function cancelOrder(req: Request, res: Response) {
  ...
  // Notify the customer if they did not cancel themselves
  if (order.cancelledBy !== "CUSTOMER") {
    await notify({
      ...
      body: `Your order ${order.publicId} was cancelled${reason ? `: ${reason}` : "."}`,
      relatedType: "Order",
  ...
  if (order.riderId && order.cancelledBy !== "RIDER") {
    await notify({
      recipientType: "RIDER",
      recipientId: String(order.riderId),
      type: "ORDER_STATUS_CHANGED",
      title: "Order cancelled",
...
export async function verifyDeliveryOtp(req: Request, res: Response) {
  ...
  if (stage === "pickup") {
    ...
    await notify({
      ...
      body: `Your order ${order.publicId} has been picked up and is on the way.`,
      relatedType: "Order",
      relatedId: String(order._id),
    });
  } else {
    ...
    // Credit the rider's completed delivery count
    await Rider.findByIdAndUpdate(order.riderId, { $inc: { completedDeliveries: 1 } });
    ...
    await notify({
      ...
      body: `Your order ${order.publicId} has been delivered. Thank you for using NagarGo!`,
      relatedType: "Order",
      relatedId: String(order._id),
    });

    // Broadcast delivery completion to admin dashboard
    broadcastToAdmins("order:status-changed", {
      orderId: String(order._id),
      publicId: order.publicId,
      status: "DELIVERED",
...
/**
 * Returns a structured JSON receipt for a completed order.
 * Safe for both customer and admin access (ownership checked for customers).
 */
export async function getOrderReceipt(req: Request, res: Response) {
  const order = await Order.findById(req.params.id)
    .populate("customerId", "name phone publicId")
    .populate("riderId", "fullName phone publicId vehicle")
    .populate("paymentId");
  ...
  if (order.status !== "DELIVERED") {
    throw new AppError("Receipt is only available for delivered orders.", 400);
  }

  res.json({
    receipt: {
      receiptNumber: order.publicId,
      issuedAt: new Date().toISOString(),
      order: {
        id: order.publicId,
        status: order.status,
        createdAt: order.createdAt,
        deliveredAt: order.deliveryOtpVerifiedAt,
