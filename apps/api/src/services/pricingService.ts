...
export async function calculatePricing({
  ...
  const estimatedMinutes = routeMinutes && routeMinutes > 0 ? Math.ceil(routeMinutes) : Math.ceil((roundedDistance / AVERAGE_SPEED_KMPH) * 60);

  const distanceFare = roundedDistance * config.perKmRate;
  const subtotal = config.baseFare + distanceFare + config.serviceFee;
